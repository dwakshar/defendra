...
class DartTokenizer {
  ...
  Map<String, int> get vocab {
    assert(
      _loaded,
      'DartTokenizer.load() must be called before accessing vocab',
    );
    return _vocab;
  }
  ...
  Map<String, List<int>> tokenize(
    ...
    final result = _tokenizeImpl(text, maxLength: maxLength);
  ...
  //   1. _cleanAndSpace  — strip controls + normalize whitespace + wrap CJK
  //                        (single pass; replaces _cleanText + _addCjkSpaces)
  //   2. whitespace-split
  //   3. _splitOnPunc    — split each token on punctuation boundaries,
  //                        but never split preserved placeholder tokens
  // ---------------------------------------------------------------------------

  // Combined _cleanText + _addCjkSpaces in a single codepoint loop.
  String _cleanAndSpace(String text) {
